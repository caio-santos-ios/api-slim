...
namespace api_slim.src.Shared.Utils
{
    public static class MongoUtil
    {
        ...
        public static BsonDocument LookupV2(string from, string[] localField, string[] foreignField, string property, dynamic[][]? filters = null, int? limit = null, bool tableShared = false, string _operator = "eq")
        {
            BsonArray match = new();
            BsonDocument letFields = new();
        ...
        public static BsonDocument IfNull(dynamic field, dynamic valueIfNull)
        {
            return new BsonDocument("$ifNull", new BsonArray { $"${field}", valueIfNull });
        }
